...
static void button2() {
	...
	static uint32_t off_time = 0xFFFFFFFF;

	uint32_t s2 = LL_GPIO_IsInputPinSet(S2_GPIO_Port, S2_Pin);
	states <<= 1;
	...
	if (Tick > off_time + LED_TIME_LONG) {
...
int main(void)
{
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  ...
	  if (Tick > last_button_tick + BUTTON_PERIOD_SHORT) {
